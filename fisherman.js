  ...
  app.post("/store", (req, res) => {
    ...
    fs.readFile(dbPath, (err, data) => {
      ...
      try {
        ...
        console.error("Error parsing db.json:", e.message);
        return res.status(500).send("Internal Server Error");
      }
...
function promptUser() {
  displayMenu();
  rl.question("Which platform shall we use? ", (selection) => {
    const platform = handleSelection(selection);
    if (platform) {
      console.log(`You selected ${platform}`);
      moveAhead(platform);
    } else {
